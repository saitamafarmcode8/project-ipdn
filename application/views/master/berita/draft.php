				...
				<table class="table table-striped table-bordered scroll-vertical mb-2">
					...
					<tbody>
						<?php $bo=1; foreach($rows as $b): 
							...
							<tr>
								<td><?= $bo++; ?></td>
								<td><?= anchor('berita/beritadraftdetail/'.$link, $b['title']); ?></td>
								<td><?= $b['namaktg']; ?></td>
								<td class="text-truncate"><?= $isi; ?>...</td>
								<td><?= tgl_indo($b['tglposting']); ?></td>
								
								<td><a href="<?= base_url('news_hapus_data/'.$b['nomorberita']); ?>" onclick="return confirm('Apa kamu yakin berita akan dihapus ?')"><i class="ft-trash danger"></i></a></td>
							</tr>
						<?php endforeach; ?>
					</tbody>
					<tfoot>
